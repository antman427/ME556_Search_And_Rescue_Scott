"""
Simple example: Single robot search in a basic environment.
Demonstrates integration of environment, robot, SLAM, vision, and ergodic control.
"""
...
from src.environments.environment import Environment, Objective
...
def main():
    print("=" * 60)
    print("Single Robot Ergodic Search Demonstration")
    print("=" * 60)
    ...
    # Initialize particle filters for each objective type
    for obj_type in ['person', 'pet']:
        detection_filter.initialize_objective(obj_type)
    
    # Create ergodic controller
    print("\n5. Initializing ergodic controller...")
    ergodic_ctrl = ErgodicController(map_size=map_size, num_basis=3, horizon=15, dt=dt)
    ...
    for step in range(max_steps):
        ...
        # Feature density (simplified - use gradient of occupancy)
        ...
        control = ergodic_ctrl.get_next_control(robot_state, target_dist)
        ...
        if step % 10 == 0:
            ...
            if len(traj) > 10:
                target_coeffs = ergodic_ctrl.compute_spatial_distribution(target_dist)
                erg_metric = ergodic_ctrl.ergodic_metric(traj[-50:], target_coeffs)
                metrics['ergodic_metric'].append(erg_metric)
    ...
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ax.set_title('Environment and Robot Trajectory')
    ...
    ax = axes[1, 1]
    plt.sca(ax)
    if len(metrics['ergodic_metric']) > 0:
        plt.plot(metrics['time'][:len(metrics['ergodic_metric'])], 
                metrics['ergodic_metric'], 'g-', linewidth=2)
        plt.xlabel('Time (s)')
        plt.ylabel('Ergodic Metric')
        plt.title('Ergodic Metric (lower = better coverage)')
